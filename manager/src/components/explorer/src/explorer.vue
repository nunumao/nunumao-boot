<template>
  <el-dialog custom-class="d1-explorer" title="" v-model="show" width="960px" :show-close="false" :close-on-click-modal="false">
    <template #title>
      <div class="d-flex justify-content-between">
        <div>资源管理</div>
        <div class="dialog-icon d-flex align-items-center">
          <div class="d-flex align-items-center ml-10 h-100" @click="close">
            <i style="font-size:14px;margin-top:2px" class="el-icon-close"></i>关闭
          </div>
        </div>
      </div>
    </template>
    ...
    <div class="upload d-flex justify-content-between">
      ...
      <div class="file border d-flex flex-column">
        <div class="action border-bottom d-flex justify-content-between">
          <template v-if="pageUpload">
            <div class="acl d-flex h-100">
              <div class="back h-100 d-flex align-items-center border-right" @click="back">
        ...
        <div class="file-list d-flex flex-wrap">
          <template v-if="!pageUpload">
            <template v-if="file.list.records.length > 0">
        ...
        <div class="file-footer d-flex justify-content-between align-items-stretch border-top">
          <template v-if="!pageUpload">
          ...
          <template v-else>
            <div class="total-progress d-flex h-100">
              ...
              <div class="info border-left d-flex align-items-center">
                ...
                <template v-if="finish"> ,已全部完成 </template>
                <template v-else>
...
export default {
  ...
  methods: {
    ...
    folderList() {
      this.folder.list = [];
      this.$axios.get(this.api + '/folder-list', { pid: this.folder.pid }).then(data => {
    ...
    readFileList() {
      ...
      this.$axios.get(this.api + '/file-list', { folderId: pid, page: this.file.list.current }).then(data => {
    ...
    // 新增目录
    folderAdd() {
      this.$axios.post(this.api + '/folder-add', { name: this.folder.newName, pid: this.folder.pid }).then(data => {
        this.folder.new = false;
        this.alert('新增成功');
        this.folderList();
      }).catch(e => {
    ...
    folderPut(data) {
      this.$axios.put(this.api + '/folder-edit', { id: data.id, name: data.name }).then(data => {
    ...
    folderRemove(id) {
      this.$axios.delete(this.api + '/folder-delete', { id }).then(data => {
        if (data.code == this.$config.SUCCESS_CODE) {
          this.alert('删除成功');
          this.folderList();
        } else {
    ...
    upload(data) {
      return new Promise((resolve, reject) => {
        axios.request({
          url: this.$config.BASE_API + this.api + '/file-upload',
          method: 'post',
          data: data,
          headers: {
