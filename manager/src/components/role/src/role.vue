<template>
  <div class="d1-components-role">
    <el-input v-model="show" readonly placeholder="请选择角色" suffix-icon="el-icon-user" @click="open"></el-input>
    <el-dialog title="选择角色" v-model="dialog" width="800px" :show-close="false" destroy-on-close @close="close">
      <div>
...
export default {
  ...
  watch: {
    value: function (v) {
      if (v) {
        this.$axios.get(this.api + '/role-detail', { id: v }).then(data => {
  ...
  methods: {
    loadList() {
      this.$axios.get(this.api + '/role-list', this.query).then(data => {
