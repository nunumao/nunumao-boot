<template>
  <div class="crm-components-chance">
    <el-input v-model="value.name" readonly placeholder="请选择商机" @click="open" />
    <el-dialog v-model="dialog" title="关联商机" width="600px" destroy-on-close :before-close="close">
      <div>
        <div class="d-flex">
          <div>
            <el-input v-model="query.search" size="small" clearable placeholder="商机名称/商机编号" />
          </div>
          <div class="ml-10">
...
export default {
  ...
  methods: {
    ...
    select(data) {
      this.$emit('update:value', { id: data.id, name: data.name });
      this.$emit('result', { id: data.id, name: data.name });
      this.close();
    }
  }
}
